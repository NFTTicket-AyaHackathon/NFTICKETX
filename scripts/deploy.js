...
async function main() {
  ...
  const NfticketContract = await hre.ethers.deployContract("NFticket");
  await NfticketContract.waitForDeployment();
  console.log("Nfticket deployed to:", NfticketContract.target);
  ...
  await hre.run("verify:verify", {
    address: NfticketContract.target,
    constructorArguments: [],
  });
}
